...
std::string DateTimeComponent::getDisplayString() const
{
	if (mDisplayRelative) {
		...
		if(dur.getDays() > 0)
			sprintf(buf, EsLocale::nGetText("%i day ago", "%i days ago", dur.getDays()).c_str(), dur.getDays());
		else if(dur.getHours() > 0)
			sprintf(buf, EsLocale::nGetText("%i hour ago", "%i hours ago", dur.getHours()).c_str(), dur.getHours());
		else if(dur.getMinutes() > 0)
			sprintf(buf, EsLocale::nGetText("%i min ago", "%i mins ago", dur.getMinutes()).c_str(), dur.getMinutes());
		else
			sprintf(buf, EsLocale::nGetText("%i sec ago", "%i secs ago", dur.getSeconds()).c_str(), dur.getSeconds());
